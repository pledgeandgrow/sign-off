...
export default function CreateInheritancePlanScreen() {
  ...
  const [activationMethod, setActivationMethod] = useState<'inactivity' | 'death_certificate' | 'manual_trigger' | 'scheduled'>('inactivity');
  ...
  const handleSave = async () => {
    ...
    if (selectedVaults.length === 0) {
      Alert.alert('Error', 'Please select at least one vault');
      return;
    }
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.headerInfo, { backgroundColor: 'rgba(139, 92, 246, 0.1)', borderColor: 'rgba(139, 92, 246, 0.3)' }]}>
          ...
          <Text style={[styles.headerInfoText, { color: colors.text }]}>
            Créez un plan pour gérer l'accès à vos coffres-forts après votre décès
          </Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          {vaults.length === 0 ? (
            <View style={[styles.emptyState, { backgroundColor: 'rgba(255, 255, 255, 0.03)', borderColor: colors.border }]}>
              <MaterialCommunityIcons name="lock-plus" size={48} color={colors.textSecondary} />
              <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
                Aucun coffre-fort disponible
              </Text>
              <Text style={[styles.emptySubtext, { color: colors.textSecondary }]}>
                Créez un coffre-fort d'abord
              </Text>
            </View>
          ) : (
            vaults.map((vault) => (
